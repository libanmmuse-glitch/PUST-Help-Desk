...
{
    $db = getDB();
    $stmt = $db->prepare('SELECT COUNT(*) FROM notifications WHERE user_id = ? AND is_read = 0 AND deleted_at IS NULL');
    $stmt->execute([$userId]);
...
{
    $db = getDB();
    $stmt = $db->prepare('UPDATE notifications SET is_read = 1, read_at = COALESCE(read_at, NOW()) WHERE id = ? AND user_id = ? AND deleted_at IS NULL');
...
{
    $db = getDB();
    $stmt = $db->prepare('UPDATE notifications SET is_read = 1, read_at = COALESCE(read_at, NOW()) WHERE user_id = ? AND deleted_at IS NULL');
    $stmt->execute([$userId]);
}
...
{
    $db = getDB();
    $stmt = $db->prepare("SELECT a.*, u.first_name, u.last_name FROM activity_logs a LEFT JOIN users u ON a.user_id = u.id AND u.deleted_at IS NULL WHERE a.deleted_at IS NULL ORDER BY a.created_at DESC LIMIT ?");
...
function getCategories(?int $departmentId = null): array
{
    $db = getDB();
    if ($departmentId) {
        $stmt = $db->prepare('SELECT * FROM categories WHERE deleted_at IS NULL AND is_active = 1 AND (department_id = ? OR department_id IS NULL) ORDER BY name');
        $stmt->execute([$departmentId]);
        return $stmt->fetchAll();
    }
    return $db->query('SELECT * FROM categories WHERE deleted_at IS NULL AND is_active = 1 ORDER BY name')->fetchAll();
}
...
{
    $db = getDB();
    $stmt = $db->prepare('SELECT id FROM departments WHERE id = ? AND is_active = 1 AND deleted_at IS NULL');
    $stmt->execute([$departmentId]);
...
{
    $db = getDB();
    return $db->query('SELECT * FROM priorities WHERE deleted_at IS NULL ORDER BY sort_order')->fetchAll();
}
...
function getAssignableUsers(int $departmentId, bool $allDepartments = false): array
{
    ...
    if ($allDepartments || userRole() === 'admin') {
        $stmt = $db->query("SELECT id, first_name, last_name, email, role, department_id
            FROM users WHERE role IN ('staff','admin') AND is_active = 1 AND deleted_at IS NULL
    ...
    $stmt = $db->prepare("SELECT id, first_name, last_name, email, role, department_id
        FROM users WHERE role IN ('staff','admin') AND department_id = ? AND is_active = 1 AND deleted_at IS NULL
...
function getAllUsers(string $role = '', int $page = 1, int $perPage = 15): array
{
    ...
    $sql = "SELECT u.id, u.student_id, u.first_name, u.last_name, u.email, u.phone, u.role,
            ...
            WHERE u.deleted_at IS NULL" . ($role ? ' AND u.role = ?' : '') . " ORDER BY u.created_at DESC LIMIT {$pagination['per_page']} OFFSET {$pagination['offset']}";
    $stmt = $db->prepare($sql);
    $stmt->execute($params);
